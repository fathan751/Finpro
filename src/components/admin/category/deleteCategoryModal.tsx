...
const DeleteCategoryModal = ({onConfirm,onClose,id}:DeleteCategoryProps) => {
    ...
    const handleDeleteCategory = async () => {
        ...
        try {
            const resDelete = await fetch(`${BASE_URL}/api/v1/delete-category/${id}`,{
                method:"DELETE",
                headers:{
    ...
    <div className="inset-0 fixed z-10 flex items-center justify-center">
        ...
        <div className="relative bg-white rounded-lg shadow flex flex-col max-w-[600px] gap-3 px-6 py-3">
            ...
            <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleDeleteCategory} className="hover:scale-125" variant={"destructive"}>Continue</Button>
            </div>
        </div>
    </div>
  )
