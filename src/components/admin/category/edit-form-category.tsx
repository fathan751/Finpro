...
const EditCategoryForm = ({categoryData,categoryId}:EditFormProps) => {
    ...
    useEffect(() => {
        if (categoryData) {
            ...
            if (categoryData.imageUrl && categoryData.imageUrl.length > 0) {
            setPreviewImg(categoryData.imageUrl)
            }
    ...
    const handleUpdateActivity = async(e:React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const editRes = await axios.post(`${BASE_URL}/api/v1/update-category/${categoryId}`,payload,{
                headers:{
