  ...
  return (
    <div>
        {[...transactions].reverse().map((item) =>(
                ...
                <div className="flex items-center gap-1 mt-2 mb-3 px-2 font-normal text-gray-700 w-full">
                    <div className="w-full">
                        <div className="flex items-center justify-between text-sm font-medium text-gray-900 truncate">
                            ...
                            <span>{item.transaction_items.map(activity => activity.title +", ")}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm font-medium text-gray-900 truncate">
                            <span>Price</span>
                        ...
                        {item.proofPaymentUrl? (
                            ...
                            <a className="underline text-blue-500" rel="noopener noreferrer" href={item.proofPaymentUrl} target="_blank">View Payment Proof</a>
                            <img src={item.proofPaymentUrl??"images/brokenimg.png"} className="size-25"
                            onError={(e) => {
                                e.currentTarget.src = "images/brokenimg.png"
                            }}/>
                            </div>
                        ):(
                            <span className="mt-3 text-sm text-gray-400 italic">No Payment Proof Uploaded</span>
                        )}
            ...
            <div className="flex items-end justify-end absolute bottom-4 right-4">
      ...
      />
                ...
                :<Link href={`/my-transaction/${item.id}`}>
